...
// PDF用HTMLを生成
function buildPdfHtml(estimate, price) {
  ...
  const detailRows = items.map((item, i) => `
    <tr style="background:${i % 2 === 1 ? '#f0f8f4' : '#fff'}">
  ...
  if (price.deadlineAdjustment > 0) {
    summaryRows += `<tr><td colspan="4" style="padding:6px 8px;border:1px solid #ddd;text-align:right">納期調整（${DEADLINE_LABELS[estimate.deadlineRate]}）</td>
  ...
  if (price.discount > 0) {
    ...
    summaryRows += `<tr><td colspan="4" style="padding:6px 8px;border:1px solid #ddd;text-align:right">${dLabel}</td>
  ...
  return `
    <div style="width:780px;font-family:'M PLUS Rounded 1c','Hiragino Maru Gothic ProN',sans-serif;color:#333;padding:32px;background:#fff">
      <!-- ページ1: 見積もり明細 -->
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:20px">
        ...
        <div style="text-align:right;font-size:12px;color:#555">
          <div>見積日：${formatDate(today)}</div>
          <div>有効期限：${formatDate(expiry)}</div>
          <div>見積番号：${estNum}</div>
      ...
      <h2 style="font-size:18px;color:#085041;border-bottom:2px solid #1D9E75;padding-bottom:8px;margin-bottom:16px">サイト仕様</h2>

      <h3 style="font-size:14px;color:#1D9E75;margin:16px 0 8px">基本情報</h3>
      <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:16px">
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;width:160px;font-weight:bold">サイト種別</td><td style="padding:6px 8px;border:1px solid #ddd">${SITE_TYPE_LABELS[estimate.siteType]}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">制作方式</td><td style="padding:6px 8px;border:1px solid #ddd">${BUILD_METHOD_LABELS[estimate.buildMethod]}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">ページ数</td><td style="padding:6px 8px;border:1px solid #ddd">トップ${topCount}P + 下層${estimate.subPageCount}P + LP${estimate.lpPageCount}P（計${totalPages}P）</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">レスポンシブ</td><td style="padding:6px 8px;border:1px solid #ddd">${estimate.responsive ? '対応する' : 'しない'}</td></tr>
      </table>

      <h3 style="font-size:14px;color:#1D9E75;margin:16px 0 8px">デザイン設定</h3>
      <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:16px">
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;width:160px;font-weight:bold">メインカラー</td><td style="padding:6px 8px;border:1px solid #ddd"><span style="display:inline-block;width:14px;height:14px;background:${estimate.colorMain};border-radius:3px;vertical-align:middle;margin-right:6px"></span>${estimate.colorMain}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">サブカラー</td><td style="padding:6px 8px;border:1px solid #ddd"><span style="display:inline-block;width:14px;height:14px;background:${estimate.colorSub};border-radius:3px;vertical-align:middle;margin-right:6px"></span>${estimate.colorSub}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">アクセント</td><td style="padding:6px 8px;border:1px solid #ddd"><span style="display:inline-block;width:14px;height:14px;background:${estimate.colorAccent};border-radius:3px;vertical-align:middle;margin-right:6px"></span>${estimate.colorAccent}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">フォント</td><td style="padding:6px 8px;border:1px solid #ddd">${FONT_LABELS[estimate.fontStyle]}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">レイアウト</td><td style="padding:6px 8px;border:1px solid #ddd">${LAYOUT_LABELS[estimate.layoutPattern]}</td></tr>
      </table>

      <h3 style="font-size:14px;color:#1D9E75;margin:16px 0 8px">選択機能</h3>
      <p style="font-size:12px;margin-bottom:16px">${activeFeatures.length > 0 ? activeFeatures.join(' / ') : 'なし'}</p>
      ...
      <!-- ページ3: 備考 -->
      <div style="page-break-before:always"></div>
      <h2 style="font-size:18px;color:#085041;border-bottom:2px solid #1D9E75;padding-bottom:8px;margin-bottom:16px">備考・補足事項</h2>

      <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:16px">
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;width:180px;font-weight:bold">デザイン修正の無料回数</td><td style="padding:6px 8px;border:1px solid #ddd">${estimate.freeRevisions || '未設定'}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">追加費用について</td><td style="padding:6px 8px;border:1px solid #ddd">仕様変更・ページ追加が発生した場合は別途お見積り</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">素材について</td><td style="padding:6px 8px;border:1px solid #ddd">写真・テキスト等の素材はお客様にご用意いただきます</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">著作権</td><td style="padding:6px 8px;border:1px solid #ddd">制作物の著作権は納品・全額入金後にお客様へ譲渡</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">保守プラン</td><td style="padding:6px 8px;border:1px solid #ddd">${SUPPORT_LABELS[estimate.supportPlan]}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">支払いタイミング</td><td style="padding:6px 8px;border:1px solid #ddd">${estimate.paymentTiming || '未設定'}</td></tr>
        <tr><td style="padding:6px 8px;border:1px solid #ddd;background:#f0f8f4;font-weight:bold">支払い方法</td><td style="padding:6px 8px;border:1px solid #ddd">${estimate.paymentMethod || '未設定'}</td></tr>
      </table>
...
export async function exportPdf(estimate, price) {
  ...
  container.innerHTML = buildPdfHtml(estimate, price);
  ...
  try {
    ...
    pdf.save(`MitsuMO_見積書_${clientName}_${formatDateShort(new Date())}.pdf`);
  } finally {
